...
type StakingRewardProps = Omit<StakingReward, NonNullable<FunctionPropertyNames<StakingReward>>>;
...
export class StakingReward implements Entity {
    ...
    static async get(id:string): Promise<StakingReward | undefined>{
        ...
        if (record){
            return StakingReward.create(record as StakingRewardProps);
        }else{
            return;
        }
    }
    ...
    static async getByAccountId(accountId: string): Promise<StakingReward[] | undefined>{
      ...
      return records.map(record => StakingReward.create(record as StakingRewardProps));
